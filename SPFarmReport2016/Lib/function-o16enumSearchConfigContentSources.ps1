function o16enumSearchConfigContentSources {
    ...
    try {
        ...
        for ($tempCnt = 0; $tempCnt -lt $script:searchsvcAppsCount ; $tempCnt ++) {
            $cmdstr = Get-SPEnterpriseSearchCrawlContentSource -SearchApplication $searchServiceAppIds[$tempCnt] | Select-Object Id | Format-Table -HideTableHeaders | Out-String -Width 1000
