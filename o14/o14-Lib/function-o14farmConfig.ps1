function o14farmConfig {
    ...
    try {
        ...
        $script:DSN = Get-ItemProperty "hklm:SOFTWARE\Microsoft\Shared Tools\Web Server Extensions\14.0\Secure\configdb" | select dsn | Format-Table -HideTableHeaders | Out-String -Width 1024
        if (-not $?) {
